{{-- resources/views/admin/project-plans/edit.blade.php --}}
@extends('layouts.app')
...
<div class="container">
    <h1>تعديل البند: {{ $projectPlan->item_name }}</h1>
    ...
    <form action="{{ route('admin.project-plans.update',['site'=>$site,$projectPlan->id]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="section-selector">
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="section-option {{ old('parent_section', $projectPlan->parent_section) == $section ? 'active' : '' }}"
            ...
            <input type="hidden" name="parent_section" id="parent_section" value="{{ old('parent_section', $projectPlan->parent_section) }}">
        </div>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <select name="item_type" id="item_type" class="form-control" required onchange="toggleParentSelection()">
                        ...
                        <option value="section" {{ old('item_type', $projectPlan->item_type) == 'section' ? 'selected' : '' }}>قسم رئيسي</option>
                        <option value="main" {{ old('item_type', $projectPlan->item_type) == 'main' ? 'selected' : '' }}>بند رئيسي</option>
                        <option value="sub" {{ old('item_type', $projectPlan->item_type) == 'sub' ? 'selected' : '' }}>بند فرعي</option>
                    </select>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <select name="parent_id" id="parent_id" class="form-control" onchange="updateParentInfo()">
                        ...
                        @foreach($mainItems as $item)
                            @if($item->id != $projectPlan->id) {{-- تجنب اختيار البند نفسه كأب --}}
                                @if($item->parent_section == (old('parent_section', $projectPlan->parent_section)))
                                    <option value="{{ $item->id }}"
                                            {{ old('parent_id', $projectPlan->parent_id) == $item->id ? 'selected' : '' }}
                    ...
                    <div class="parent-info" id="parent-info">
                        @if(old('parent_id', $projectPlan->parent_id))
                            @php
                                $selectedParent = $mainItems->firstWhere('id', old('parent_id', $projectPlan->parent_id));
                            @endphp
                            @if($selectedParent)
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <input type="number" name="item_number" id="item_number" class="form-control"
                           value="{{ old('item_number', $projectPlan->item_number) }}" min="1" max="100">
                </div>
            </div>
            <div class="col-md-9">
                <div class="form-group">
                    ...
                    <input type="text" name="item_name" id="item_name" class="form-control"
                           value="{{ old('item_name', $projectPlan->item_name) }}" >
                </div>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="requirements" id="requirements" class="form-control" rows="2">{{ old('requirements', $projectPlan->requirements) }}</textarea>
        </div>
            ...
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <input type="date" name="start_date" id="start_date" class="form-control"
                           value="{{ old('start_date', $projectPlan->start_date ? $projectPlan->start_date->format('Y-m-d') : '') }}" onchange="calculateDuration()">
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <input type="date" name="end_date" id="end_date" class="form-control"
                           value="{{ old('end_date', $projectPlan->end_date ? $projectPlan->end_date->format('Y-m-d') : '') }}" onchange="calculateDuration(); updateStatus()">
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <input type="date" name="actual_end_date" id="actual_end_date" class="form-control"
                           value="{{ old('actual_end_date', $projectPlan->actual_end_date ? $projectPlan->actual_end_date->format('Y-m-d') : '') }}" onchange="updateStatus()">
                </div>
            </div>
        </div>
            ...
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <input type="text" name="duration" id="duration" class="form-control"
                           value="{{ old('duration', $projectPlan->duration) }}" readonly>
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <select name="department" id="department" class="form-control">
                        ...
                        <option value="إدارة التنفيذ" {{ old('department', $projectPlan->department) == 'إدارة التنفيذ' ? 'selected' : '' }}>إدارة التنفيذ</option>
                        <option value="إدارة التطوير" {{ old('department', $projectPlan->department) == 'إدارة التطوير' ? 'selected' : '' }}>إدارة التطوير</option>
                        <option value="الإدارة العليا" {{ old('department', $projectPlan->department) == 'الإدارة العليا' ? 'selected' : '' }}>الإدارة العليا</option>
                        <option value="-" {{ old('department', $projectPlan->department) == '-' ? 'selected' : '' }}>-</option>
                    </select>
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <select name="responsible" id="responsible" class="form-control">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->name }}" {{ old('responsible', $projectPlan->responsible) == $user->name ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                        <option value="-" {{ old('responsible', $projectPlan->responsible) == '-' ? 'selected' : '' }}>-</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="notes" id="notes" class="form-control" rows="3">{{ old('notes', $projectPlan->notes) }}</textarea>
        </div>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control" required>
                        @php
                            $currentStatus = str_replace('status-', '', $projectPlan->status_class);
                        @endphp
                        <option value="notstarted" {{ old('status', $currentStatus) == 'notstarted' ? 'selected' : '' }}>لم يبدأ</option>
                        <option value="inprogress" {{ old('status', $currentStatus) == 'inprogress' ? 'selected' : '' }}>قيد التنفيذ</option>
                        <option value="completed" {{ old('status', $currentStatus) == 'completed' ? 'selected' : '' }}>مكتمل</option>
                    </select>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="number" name="sort_order" id="sort_order" class="form-control"
                           value="{{ old('sort_order', $projectPlan->sort_order) }}" min="0">
                </div>
            </div>
        </div>
...
<script>
    ...
    function filterParentsBySection(section) {
        ...
        // إعادة تعيين التحديد
        const currentParentId = "{{ old('parent_id', $projectPlan->parent_id) }}";
        if (currentParentId) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // تصفية الأباء حسب القسم الحالي
        const currentSection = document.getElementById('parent_section').value;
        filterParentsBySection(currentSection);
        ...
        // حساب المدة تلقائياً عند التحميل إذا كانت هناك تواريخ
        const startDate = document.getElementById('start_date').value;
        const endDate = document.getElementById('end_date').value;
        if (startDate && endDate) {
            calculateDuration();
