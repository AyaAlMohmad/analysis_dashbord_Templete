...
@section('content')
    ...
    <div id="report-content">
                ...
                <div class="projects-logos">
                    ...
                    @foreach ($localSites as $site)
                        @if ($site->logo_path_white)
                            <div class="project-logo">
                                <img src="{{ asset('storage/' . $site->logo_path_white) }}" alt="{{ $site->name }}"
                                    style="height: 50px;">
                            </div>
                        @endif
                    @endforeach
        ...
        @if (in_array('jeddah', $requestSites))
            <div class="page-section">
                <div class="khozam-section"
                    style="background-color: #f9f6f2; padding: 60px 20px; direction: rtl; position: relative; display: flex; align-items: center; justify-content: space-between; font-family: 'Arial', sans-serif; flex-wrap: wrap;">
                    <div style="position: absolute; top: 30px; right: 30px;">
        ...
        @endif
        <div class="project-section" style="margin-bottom: 50px;">
            @if (in_array('jeddah', $requestSites) && in_array('colored_map', $requestSections))
                <div class="page-section">@include('comprehensive.colored_map', [
                    'data' => $jeddahColoredMap,
                    'project_name' => 'أزيان جدة',
                ])</div>
            @endif
            @if (in_array('jeddah', $requestSites) && in_array('reserved_report', $requestSections))
                <div class="page-section">@include('comprehensive.reserved_report', [
                    'data' => $jeddahData,
                    'project_name' => 'أزيان جدة',
                ])</div>
            @endif
            @if (in_array('jeddah', $requestSites) && in_array('contracts_report', $requestSections))
                <div class="page-section">@include('comprehensive.contracts_report', [
                    'data' => $jeddahDataContract,
                    'project_name' => 'أزيان جدة',
                ])</div>
            @endif
            @if (in_array('jeddah', $requestSites) && in_array('status_item', $requestSections))
                <div class="page-section">@include('comprehensive.status_item', [
                    'statusData' => $bashaerStatusData,
                    'project_name' => 'أزيان جدة',
                ])</div>
            @endif
            @if (in_array('jeddah', $requestSites) && in_array('project_summary', $requestSections))
                <div class="page-section">@include('comprehensive.project_summary', [
                    'data' => $jeddahProjectSummary,
                    'project_name' => 'أزيان جدة',
                ])</div>
            @endif
            @if (in_array('jeddah', $requestSites) && in_array('unitStages', $requestSections))
                <div class="page-section no-print">@include('comprehensive.unitStages', [
                    'unitStages' => $jeddahUnitStages,
                    'project_name' => 'أزيان جدة',
                ])</div>
            @endif
            @if (in_array('jeddah', $requestSites) && in_array('unitStatisticsByStage', $requestSections))
                <div class="page-section">
                    @include('comprehensive.unitStatisticsByStage', [
                        'unitStats' => $unitDetailsByStageResultJeddah,
                        'project_name' => 'أزيان جدة',
                    ])
                </div>
            @endif
            @if (in_array('jeddah', $requestSites) && in_array('visits_payments_contracts', $requestSections))
                <div class="page-section">@include('comprehensive.visits_payments_contracts', [
                    'data' => $jeddahVPCData,
                    'project_name' => 'أزيان جدة',
                ])</div>
            @endif
            {{-- @if (in_array('jeddah', $requestSites) && in_array('disinterest_reasons', $requestSections))
            <div class="page-section">@include('comprehensive.disinterest_reasons', [
                'data' => $jeddahDisinterestReasons,
                'project_name' => 'أزيان جدة',
            ])</div>
            @endif --}}
            @if (in_array('jeddah', $requestSites) && in_array('unit_sales', $requestSections))
                <div class="page-section">@include('comprehensive.unit_sales', [
                    'data' => $jeddahUnitSales,
                    'project_name' => 'أزيان جدة',
                ])</div>
            @endif
            @if (in_array('jeddah', $requestSites) && in_array('source_stats', $requestSections))
                <div class="page-section">@include('comprehensive.source_stats', [
                    'data' => $jeddahUnitStatisticsByStage,
                    'project_name' => 'أزيان جدة',
                ])></div>
            @endif
            @if (in_array('jeddah', $requestSites) && in_array('monthly_appointments', $requestSections))
                <div class="page-section">@include('comprehensive.monthly_appointments', [
                    'data' => $jeddahMonthlyAppointments,
                    'project_name' => 'أزيان جدة',
                ])</div>
            @endif
            @if (in_array('jeddah', $requestSites) && in_array('targeted_report', $requestSections))
                <div class="page-section">@include('comprehensive.targeted_report', [
                    'data' => $jeddahTargetedReport,
                    'project_name' => 'أزيان جدة',
                ])</div>
            @endif

        </div>
        @if ($localSectionResults)
         @foreach ($localSectionResults as $siteId => $siteData)
    @php
        $site = $siteData['site'] ?? null;
        $sections = $siteData['sections'] ?? [];

        // إذا كان $site null، إنشاء كائن افتراضي
        if (!$site) {
            $site = (object) [
                'id' => $siteId,
                'name' => 'موقع غير معروف',
                'logo_path' => null,
                'logo_path_white' => null,
                'map_path' => null,
            ];
        }
    @endphp
    ...
    <div class="page-section">
        <div class="khozam-section"
            ...
            <div style="flex: 2; text-align: center; padding: 20px; max-width: 100%;">
                <div style="margin-bottom: 30px;">
                    @if (!empty($site->logo_path) && Storage::disk('public')->exists($site->logo_path))
                        <img src="{{ asset('storage/' . $site->logo_path) }}" alt="{{ $site->name }}" style="height:200px;">
                    @else
                        <h1 style="color: #8b4513; font-size: 2.5em; margin: 0;">
                            {{ $site->name }}
                        </h1>
                    @endif
                </div>

                <h2 style="color: #8b4513; font-size: 1.8em; margin: 10px 0;">
                    التقرير الشامل
                </h2>
                <p style="color: #666; font-size: 1.2em; margin: 5px 0;">
                    للفترة من {{ $fromDate ?? 'تاريخ البداية' }} إلى {{ $toDate ?? 'تاريخ النهاية' }}
                </p>
            </div>

            @if (!empty($site->logo_path_white) && Storage::disk('public')->exists($site->logo_path_white))
        ...
        <div class="project-section" style="margin-bottom: 50px;">
            ...
            @if (in_array('colored_map', $requestSections) && isset($sections['colored_map']))
                <div class="page-section">
                    @include('comprehensive.colored_map', [
            ...
            @if (in_array('status_item', $requestSections) && isset($sections['status_item']))
                <div class="page-section">
                    @include('comprehensive.status_item', [
                        'statusData' => $sections['status_item']['data'] ?? [],
                        'project_name' => $site->name,
                        'logo' => $site->logo_path,
                    ])
                </div>
            ...
            @if (in_array('project_summary', $requestSections) && isset($sections['project_summary']))
                <div class="page-section">
                    @include('comprehensive.project_summary', [
                        'summaryData' => $sections['project_summary']['data'] ?? [],
                        'project_name' => $site->name,
                        'logo' => $site->logo_path,
                    ])
                </div>
            ...
            @if (in_array('unitStages', $requestSections) && isset($sections['unitStages']))
                <div class="page-section">
                    @include('comprehensive.unitStages', [
                        'unitStagesData' => $sections['unitStages']['data'] ?? [],
                        'project_name' => $site->name,
                        'logo' => $site->logo_path,
                    ])
                </div>
            ...
            @if (in_array('unitStatisticsByStage', $requestSections) && isset($sections['unitStatisticsByStage']))
                <div class="page-section">
                    @include('comprehensive.unitStatisticsByStage', [
                        'unitStatsData' => $sections['unitStatisticsByStage']['data'] ?? [],
                        'project_name' => $site->name,
                        'logo' => $site->logo_path,
                    ])
                </div>
            ...
            @if (in_array('visits_payments_contracts', $requestSections) && isset($sections['visits_payments_contracts']))
                <div class="page-section">
                    @include('comprehensive.visits_payments_contracts', [
                        'visitsData' => $sections['visits_payments_contracts']['data'] ?? [],
                        'project_name' => $site->name,
                        'logo' => $site->logo_path,
                    ])
                </div>
            ...
            {{-- @if (in_array('disinterest_reasons', $requestSections) && isset($sections['disinterest_reasons']))
                <div class="page-section">
                    @include('comprehensive.disinterest_reasons', [
                        'disinterestData' => $sections['disinterest_reasons']['data'] ?? [],
                        'project_name' => $site->name,
                        'logo' => $site->logo_path,
                    ])
                </div>
            ...
            @if (in_array('unit_sales', $requestSections) && isset($sections['unit_sales']))
                <div class="page-section">
                    @include('comprehensive.unit_sales', [
                        'salesData' => $sections['unit_sales']['data'] ?? [],
                        'project_name' => $site->name,
                        'logo' => $site->logo_path,
                    ])
                </div>
            ...
            @if (in_array('source_stats', $requestSections) && isset($sections['source_stats']))
                <div class="page-section">
                    @include('comprehensive.source_stats', [
                        'data' => $sections['source_stats']['data'] ?? [],
                        'project_name' => $site->name,
                        'logo' => $site->logo_path,
                    ])
                </div>
            ...
            @if (in_array('monthly_appointments', $requestSections) && isset($sections['monthly_appointments']))
                <div class="page-section">
                    @include('comprehensive.monthly_appointments', [
                        'appointmentsData' => $sections['monthly_appointments']['data'] ?? [],
                        'project_name' => $site->name,
                        'logo' => $site->logo_path,
                    ])
                </div>
            ...
            @if (in_array('targeted_report', $requestSections) && isset($sections['targeted_report']))
                <div class="page-section">
                    @include('comprehensive.targeted_report', [
