    ...
    public function log(Request $request, $site)
    {
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                $message = "Data updated successfully. Processed: {$processedCount}";
                if ($errorCount > 0) {
...
private function extractArabicText($text)
{
    ...
    {
        $logs = LeadsLog::where('site', $site)
            ->orderBy('created_at', 'desc')
            ->paginate(10);

        return view('leads.logs', compact('logs', 'site'));
    }

    public function statistics($site)
    {
        // Use database aggregation for better performance
        $dailyCounts = LeadsLog::where('site', $site)
        ...
        $recentLogs = LeadsLog::where('site', $site)
            ->orderBy('created_at', 'desc')
